...
//		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().getClass().equals(JFormattedTextField.class)) {
			...
			tf.transferFocus();
			try {
			    value = new String(Base64.encodeBase64(String.valueOf(((JPasswordField)e.getSource()).getPassword()).getBytes()));
//			    log.debug(Base64.decodeBase64(value.getBytes()));
//			    log.debug(new String(Base64.decodeBase64(value.getBytes())));
//			    log.debug(String.valueOf(((JPasswordField)e.getSource()).getPassword()).getBytes());
//			    log.debug(String.valueOf(((JPasswordField)e.getSource()).getPassword()));
//			    log.debug(value);
			} catch (Exception e1) {
...
//					log.debug("Focus action performed "+actCom+" / value: "+value);
    			...
    			if (actCom.equals("/bat/isotope/db/sql-import/cycle")||actCom.equals("/bat/isotope/db/sql/cycle")) {
...
//    			log.debug("Set DB connection to "+value);
    			if (actCom.equals("DB")) {				
    			    db_change=true;
    			    Setting.getElement("/bat/isotope/db").getAttribute("active").setValue(Boolean.toString(bool));
    			    log.debug("DB mode only activated ("+Setting.getActive("/bat/isotope/db")+")");
    			    if (Setting.getActive("/bat/isotope/db/sql")) {
        			    main.db = new IODb(main);
            			    this.panel3.dbPanel();
            			    log.debug("MySql NT is set active.");
    			    } else if (Setting.getActive("/bat/isotope/db/sql-import")) {
        			    main.db = new DbMySqlConnect(main);
            			    this.panel3.dbPanel();        				
            			    log.debug("MySql import is set active.");
    			    } else {
        			    Setting.getElement("/bat/isotope/db/sql").getAttribute("active").setValue(Boolean.toString(bool));
        			    main.db = new IODb(main);
        			    this.panel3.dbPanel();
            			    log.debug("MySql NT is set active (no DB was active!).");
   			    }
    			    main.db = new IODb(main);
    			    this.panel3.dbPanel();
    			    log.debug("DB connection activated");
    			} else if (actCom.equals("File")) {				
    			    db_change=true;
    			    Setting.getElement("/bat/isotope/db").getAttribute("active").setValue(Boolean.toString(!bool));
    			    main.db = new IODb(main);
    			    main.db=null;
    			    Setting.db=null;
    			    this.panel3.dbPanel();
    			    log.debug("File mode only activated ("+Setting.getActive("/bat/isotope/db")+")");
    			} else if (actCom.equals("MySQL NT")) {				
    			    db_change=true;
    			    Setting.getElement("/bat/isotope/db/sql").getAttribute("active").setValue(Boolean.toString(bool));
    			    Setting.getElement("/bat/isotope/db/sql-import").getAttribute("active").setValue(Boolean.toString(!bool));
    			    main.db = new IODb(main);
    			    this.panel3.dbPanel();
    			    log.debug("MySql NT selected");
    			    ...
    			    main.db = new DbMySqlConnect(main);
    			    this.panel3.dbPanel();
    			    log.debug("MySql import selected");
    			} 
			    this.panel3.dbPanel();
			    this.panel3.repaint();
...
if (c instanceof JFormattedTextField) {
	...
	public void windowClosing(WindowEvent e) {
		Setting.save();
		log.debug("Settings are saved!");
		if (db_change==true) {
        		main.reInit();
		}

        	main.data.loadSetting();
        	main.dataRecalc();
...
class PanelCalc extends JPanel {
    ...
    public PanelCalc(Preferences pref) {
	    ...
	    ftf = new JFormattedTextField(Setting.getFloat(path));
		ftf.setActionCommand(path);
		ftf.addActionListener(pref);
		ftf.setToolTipText(path);
		ftf.addFocusListener(pref);
	    ...
	    nf.setAllowsInvalid(true);
	    ftf = new JFormattedTextField(nf);
	    ftf.setValue(Setting.getFloat(path));
		ftf.setActionCommand(path);
		ftf.addActionListener(pref);
		ftf.setToolTipText(path);
		ftf.addFocusListener(pref);
	    ...
	    if (Setting.isotope.contains("C14")) {
		    des.add("Nominal F¹⁴C");
		    label = new JLabel("Nominal F¹⁴C for std");
	    } else {
	    	des.add("Nominal ratio");
	    }
	    label = new JLabel("Nominal ratio for std");
	    label.setToolTipText("Nominal ratio/F¹⁴C of standards (every std is normalised to this value)." +
...
class PanelFile extends JPanel {
    ...
    public PanelFile(Preferences pref) {
	    ...
	    button.setName(path);
	    button.setText(Setting.getString(path));
	    button.setForeground(ftf.getForeground());
	    button.setBorder(ftf.getBorder());
//	    button.setBorderPainted(true); ; 
	    button.setBackground(ftf.getBackground());
...
class PanelDb extends JPanel {
    ...
    public void dbPanel() {
	    ...
	    cp = new JPanel(new GridLayout(0,2));
		des.add("DB connection");
	    	    
	    JRadioButton dbButton = new JRadioButton("Access DB");
	    dbButton.setActionCommand("DB");
	    path="/bat/isotope/db";
	    dbButton.setSelected(Setting.getActive(path));
	    dbButton.addActionListener(pref);	    
	    cp.add(dbButton);
	    
	    JRadioButton fButton;
	    fButton = new JRadioButton("No DB, file only");
	    fButton.setActionCommand("File");	    
	    path="/bat/isotope/db";
	    fButton.setSelected(!Setting.getActive(path));
	    fButton.addActionListener(pref);
	    cp.add(fButton);
	    ...
	    label = new JLabel("");
			...
			JRadioButton thirdButton = new JRadioButton("SQL NT");
		    thirdButton.setActionCommand("MySQL NT");
		    path="/bat/isotope/db/sql";
		    thirdButton.setSelected(Setting.getActive(path));
		    thirdButton.addActionListener(pref);
		    cp.add(thirdButton);
		    
		    JRadioButton firstButton = new JRadioButton("SQL Import");
		    firstButton.setActionCommand("MySQL Import");
		    path="/bat/isotope/db/sql-import";
		    firstButton.setSelected(Setting.getActive(path));
...
//			ftf.addActionListener(pref);
		    ...
		    pwf = new JPasswordField(new String(Base64.decodeBase64(Setting.getString(path).getBytes())));
			pwf.setActionCommand(path);
			pwf.addActionListener(pref);
			cp.add(pwf);
...
class PanelUI extends JPanel {
    ...
    public PanelUI(Preferences pref) {
        ...
        label = new JLabel("Sort of passes and mean");
		label.setToolTipText("Sort of samples (0: label sorted / 1: position sorted");
	    cp.add(label);
...
class PanelColor extends JPanel {
    ...
    public PanelColor(Preferences pref) {
	    ...
	    button.setText(label.getText()); 	    
	    button.setForeground(Setting.getColor(path)); 	    	    button.setBorderPainted(true); ; 
	    button.setBackground(Setting.getColor(path));
	    button.addActionListener(pref);
	    button.setActionCommand(path);
		cp.add(button);
