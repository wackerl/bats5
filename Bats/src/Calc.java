 ...
 */
abstract class Calc {
    private static final Logger log = LogManager.getLogger("Calc");
	...
	protected static double MICRO_A = 6.24150907446076E12;
	...
	protected ArrayList<Run> runStdActive = new ArrayList<Run>();					// std14 run
...
//					log.debug(runListR.get(k).run);
					try {if (runListR.get(k).run.compareToIgnoreCase(corrList.get(i).lastRun)>0) {
						i++;
						try {
							corrList.get(i).firstRun = runListR.get(k).run;
						} catch (IndexOutOfBoundsException e) {;}
					}
					}catch(NullPointerException e){log.debug(i);}
...
//			log.debug("                                  "+deadtime);
	...
	protected void initCorrection(){	
		runStd = new ArrayList<Run>();
		runStdActive = new ArrayList<Run>();
		allStd = new ArrayList<DataSet>();
		allStdR = new ArrayList<DataSet>();
		
		runBlank = new ArrayList<Run>();
		allBlank = new ArrayList<DataSet>();
		allBlankR = new ArrayList<DataSet>();			

		runIso = new ArrayList<Run>();
		
		for (int k=0; k<sampleList.size(); k++){
//			log.debug("Correction -- "+sampleList.get(k).label+": "+sampleList.get(k).sampletype);
			if (stdNom.isBl(sampleList.get(k).type)){
				ArrayList<Run> labelRuns = runLabelList.get(k);
...
//				log.debug("Is std: "+sampleList.get(k).sampletype);
				...
				for(int j=0; j<labelRuns.size(); j++) {
					if(labelRuns.get(j).active==true) {
						runStdActive.add(labelRuns.get(j));
					}
				}
			}
		}	
...
//					log.debug(corrList.get(k).lastRun);
	...
	private void magInfo(ArrayList<Run> runListR){
		try { firstRun = runListR.get(0).run;}
		catch(IndexOutOfBoundsException e) {log.error("magInfo: magazine");}
		try { lastRun = runListR.get(runListL.size()-1).run;}
		catch(IndexOutOfBoundsException e) {log.error("magInfo: last run");}
		try { 
			if (runListR.get(0).sample.magazine==null) {
				Setting.magazine = "";
				Setting.no_data = true;
			} else {
				magazine = runListR.get(0).sample.magazine;
...
//			}
	...
	public ArrayList<DataSet> get(String order){
		...
		if (order.equals("runAll")){
			...
			list = runStd;
		}else if (order.equals("runStdActive")){
			list = runStdActive;
...
//	
	...
	protected Sample initSampleCalc(Sample sam) {
		...
		try {sample.active = sam.active;}
		catch (NullPointerException e) {sample.active = true;}
...
//	abstract protected void calcCurrent();
	 ...
	 * @param runs 
	 */		
	abstract protected void runPreCalc(Run run);
	/**
	 * Pre-calculate run mean (with blank and std correction)
	 * @param runs 
	 */		
