...
public class CalcDefault extends Calc{
	...
	private static final Logger log = LogManager.getLogger("CalcDefault");
...
/*				log.debug(corr.isoFact);
				...
				log.debug(run.iso_err);
				log.debug(run.get("isobar_err"));
...
*/				run.bg = corr.isoFact*(Double)run.get(isobar)+corr.constBG*run.runtime;
			...
			catch (NullPointerException e) {run.a_err = null;log.debug(e);}
...
//			catch (NullPointerException e) {run.ba_cur_sig = null;}
			...
			try {run.r_cor_err = Math.sqrt(Math.pow(run.bg_err,2)
