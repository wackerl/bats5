...
{
    private static final Logger log = LogManager.getLogger("DbCycle");
	
	private boolean ora, sql;
	private Document doc;
	private Connection conn;
	
	private String cycle_t;
	private String run_t;
...
//		ora = Setting.getElement("/bat/isotope/db/ora").getAttribute("active").getBooleanValue();
		sql = Setting.getElement("/bat/isotope/db/sql").getAttribute("active").getBooleanValue();
//		if (ora) {
//			cycle_t = "calcana";
//			run_t = "calcproto";
//		} else {
			cycle_t = Setting.getString("/bat/isotope/db/"+Setting.db_name+"/cycle_t");
			run_t = Setting.getString("/bat/isotope/db/"+Setting.db_name+"/run_t");
//		}
...
//				log.debug(resultSet.getString("a"));
	...
	public ResultSet queryCycles(String run, ArrayList<String> nameList){
		...
		try{
			...
			if(sql) {
			    query="SELECT editallowed, sample_nr FROM "+run_t+
				    " WHERE run='"+run+"'";
			    ResultSet res = stmt.executeQuery(query);
			    res.next();
			    if (res.getInt("editallowed")>1) {
				editallowed=false;
			    }			    
			}
...
//			log.debug("sql querry to get cycles was successful: "+query);
	     ...
	     * @param active 
	     */
	    public void setActive(String run, int cycle, boolean active) {
	    	if (ora) {
	    		String query="";
	    		if (active) {
	    		...
	    		try {
		    		String query = "call "+Setting.getString("/bat/isotope/db/"+Setting.db_name+"/cycle_enable")+
		    		"("+active+",'"+run+"','"+cycle+"')";
		    		log.debug(query);
					Statement stmt = conn.createStatement ();
				    stmt.setQueryTimeout(10);
 ...
 * @param active 
 */
public void setActive(String run, int cycle1,  int cycle2, boolean active) {
		try {
	    		String query = "call "+Setting.getString("/bat/isotope/db/"+Setting.db_name+"/cycles_enable")+
	    		"("+active+",'"+run+"','"+cycle1+"','"+cycle2+"')";
	    		log.debug(query);
				Statement stmt = conn.createStatement ();
			    stmt.setQueryTimeout(10);
				stmt.execute(query);
...
//				log.info("object "+name+" not available!"); // this is not necessary, because not all fields are available
	...
	public void set( Object value, String name ) {
		try {
			...
			else if (name.equals("tra")) {tra = ((Double) value)/100;}
	...
	public void setValue( String value, String name ) {
		try {
			...
			else if (name.equals("label")) {label = value;}
			else if (name.equals("cycle")) {cycle = Integer.valueOf(value);}
			else if (name.equals("commentinfo")) {commentinfo = value;}
			else if (name.equals("userlabel")) {userlabel = value;}
...
//				log.debug(df.format(new Date()));
			...
			else if (name.equals("tra")) {tra = Double.valueOf(value)/100;}
