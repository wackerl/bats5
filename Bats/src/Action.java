...
public class Action 
{
	...
	public void exec(String text){
        if (text == "calibMan") {
    		...
    		log.debug("execute: "+text);
//    		main.calib.statistic(16.0,1,2);
//    		main.calib.statistic(32.0,1,2);
//    		main.calib.statistic(128.0,1,2);
//    		main.calib.statistic(256.0,1,2);
//    		main.calib.statistic2(32,30);
    		main.calib.statistic2(128,30);
        }   
...
//        		main.data.corrList.get(i).constBlErr=bg[4];
		...
		main.dataRecalc2();
        	log.debug("Changed constant contamination.");
         }
        ...
        else if (text == "Save calibration") {
   	log.debug("execute: "+text);
			main.tba.update("save", true);
			IOFile.saveCalib(main.calib.calArr);
			log.debug("Saved file: "+Setting.magDir+"/"+main.data.calcSet+".xls");
			main.tba.update("saved .xls", false);
    }
...
//	        	}
    		...
    		log.debug("execute: "+text);
    		if (JOptionPane.showInputDialog("For admins only!").equals("vip")) {
    			Setting.reset();
    		}
        } 
	 		...
	 		if (run1!=null) {
		 		...
		 		if (run2!=null) {
		 			for (int k=0; k<main.data.runListR.size(); k++) {		 			    
		 				if (main.data.runListR.get(k).run.compareToIgnoreCase(run1)>=0 && main.data.runListR.get(k).run.compareToIgnoreCase(run2)<=0) {
							if (Setting.getActive("/bat/isotope/db")) {
        		 				    	if (main.db.runTrue(main.data.runListR.get(k), false)) {
        		 				    	    main.data.runListR.get(k).active=false;			
                        		        		    if ((main.data.runListR.get(k)).sample.runs==1) {
                        		        			(main.data.runListR.get(k)).sample.set(false, "active");	
                        		        		    }		        			    		        			   	
                        					} else {
                        					    log.warn("Could not deactive/active run "+(main.data.runListR.get(k)).run);
                        					}
							} else {
		 				    	    main.data.runListR.get(k).active=false;			
                		        		    if ((main.data.runListR.get(k)).sample.runs==1) {
		 				...
		 				if (Setting.getActive("/bat/isotope/db")) {
                					if (main.db.runTrue(main.data.runListR.get(k), true)) {
                						main.data.runListR.get(k).active=true;
                    		        		    	if ((main.data.runListR.get(k)).sample.runs==0) {
                    		        		    	    (main.data.runListR.get(k)).sample.active=true;	
                    		        		    	}		        			    		        			   	
                					}
		 				}else {
							main.data.runListR.get(k).active=true;
...
//		});
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
		    log.debug("Try to open file of the following tipe:"+fc.getFileFilter().getDescription());
			if (fc.getFileFilter().getDescription()=="DBXL file"){
				...
				String message = String.format( "Fatal error: File filter does not exist! ("+fc.getFileFilter().getDescription()+")");
				JOptionPane.showMessageDialog( null, message );
				log.fatal(message);
 			}
		}else{
	...
	private boolean openHV_new() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			main.tba.update("open", true);
			log.debug("HV-dir opened: "+fc.getSelectedFile().toString());
			return IOFile.openHV_new(fc.getSelectedFile(), main.data);
		}else{
		    log.debug("Open command cancelled by user.");
		    return false;
		}
	...
	private Double[] bgFrame(Corr corr) {
		...
		namePanel.setLayout(new GridLayout(0, 1));
 		// Create the labels
		JLabel labelBG = new JLabel("Counts per second:   ", SwingConstants.RIGHT);
		JLabel  labelErr = new JLabel("  error (abs):   ", SwingConstants.RIGHT);
    	...
    	if (Setting.isotope.contains("C14")){
	        ...
	        for (int i=0; i<main.data.sampleList.size(); i++) {
	        	try {
	            	...
	            	if (Setting.getString("/bat/calib/curve").length()>20)
	            	    sample.set(Setting.getString("/bat/calib/curve").substring(0,19),"cal_curve");
	            	else {
	            	    sample.set(Setting.getString("/bat/calib/curve"),"cal_curve");
	            	}
