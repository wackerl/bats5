...
public class Calc14C extends Calc{
	...
	private static final Logger log = LogManager.getLogger(Calc14C.class);
...
//		else { sample.active=true; }
		...
		try {sample.timedat = runs.get(0).timedat;}
		catch (NullPointerException e) {sample.timedat = null;}
		catch (java.lang.IndexOutOfBoundsException e) {sample.timedat = null;}
	...
	protected Sample sampleCalc(Sample sample, ArrayList<Run> runL, ArrayList<Corr> corrList) {
		...
		// calculate ra_bl
		if (stdNom.isBl(sample.type.toLowerCase()) 
		...
		try {sample.f14c = sample.ra_fin / Math.pow(sample.ba_fin,2) * Math.pow(0.975,2)-sample.prep_bl;}
		catch (NullPointerException e) {sample.f14c= null;}
//		log.debug("sample.ra_fin: "+sample.ra_fin);
//		log.debug("sample.f14c: "+sample.f14c);
//		log.debug("err: "+sample.ra_fin_err);
//		log.debug("sample.prep_bl: "+sample.prep_bl);
		try {sample.f14c_err = Math.sqrt(Math.pow(sample.ra_fin_err/sample.ra_fin,2) 
				+ 2*Math.pow(sample.ba_fin_err/sample.ba_fin,2))*sample.f14c;}
		catch (NullPointerException e) {sample.f14c_err = null;}
//		log.debug("err2: "+sample.f14c_err );
		try {sample.f14c_sig = Func.sigmaTA(runs, "fM");}
		catch (NullPointerException e) {		
			try {sample.f14c_sig = sample.ra_fin_sig;}
			catch (NullPointerException e2) {sample.f14c_sig = null;}
		}
		
		try {sample.age = mean_life * Math.log(sample.f14c);}
		catch (NullPointerException e) {sample.age = null;}
		try {sample.age_err = -mean_life * (sample.f14c_err/sample.f14c);}
	...
	protected void runPreCalc(Run run) {
...
//				run.runtime=-1.0;
			...
			try {
				...
				run.bg_err = Math.sqrt(Math.pow(corr.isoErr*(Double)run.get(isobar),2)
					+ Math.pow(corr.isoFact*run.iso_err,2) + Math.pow(corr.constBGErr*run.runtime,2));
...
//				log.info("Isobar "+isobar+" does not exist!");
			...
			catch (NullPointerException e) {run.ra_bg = 0.0;}
			...
			catch (NullPointerException e) { run.ra_bg_err = 0.0;}	
			...
			try {run.tra = run.a / charge / run.ana;}
			catch (NullPointerException e) {;}	
	...
	protected void runCalc(ArrayList<Run> runs, ArrayList<Corr> corrList) {
		...
		for (int i=0; i<runs.size(); i++)
		{
			...
			if (stdNom.isBl(run.sample.type.toLowerCase()) 
			...
			try {run.f14c = run.ra_fin / Math.pow(run.ba_fin,2) * Math.pow(0.975,2)-run.sample.prep_bl;}
			catch (NullPointerException e) {run.ra_fin = null;}
			try {run.f14c_err = Math.sqrt(Math.pow(run.ra_fin_err/run.ra_fin,2) 
					+ 2*Math.pow(run.ba_fin_err/run.ba_fin,2))*run.f14c;}
			catch (NullPointerException e) {run.ra_fin_err = null;}
			try {run.f14c_sig = run.ra_fin_sig/run.ra_fin*run.f14c;}
			catch (NullPointerException e) {run.f14c_sig = null;}
			
			try {run.age = mean_life * Math.log(run.f14c);}
			catch (NullPointerException e) {run.age = null;}
			try {run.age_err = -mean_life * run.f14c_err / run.f14c;}
