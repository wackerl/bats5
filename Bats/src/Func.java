 ...
 */
public class Func{
	private static final Logger log = LogManager.getLogger("Func");
...
//			log.debug(name);
	...
	public static double[] getMeanBl_ra_fin(ArrayList<Run> runs, ArrayList<Corr> corrList) throws NullPointerException {
		...
		result[0] = s7 / weight;  // mean value		
		result[1] = Math.sqrt(s6+s8)/weight; // error
		for (int i=0; i<runs.size(); i++) {
			if (runs.get(i).active) {
		...
		if (k>1) {
			result[2] = Math.sqrt(dev/(weight*(k-1)));  // sigma
		} else if (k==0) {
			result[2] = 0.0;
		} else {
			result[2] = sig_temp;
...
//			try {log.debug("label: "+runs.get(0).sample.label);}
//			catch (Exception e) {;}
//			log.debug("s1: "+s1);
//			log.debug("s4: "+s4);
//			log.debug("corr.blank.ra_bg_err: "+corr.blank.ra_bg_err);
//			log.debug("s4: "+weight);
			s7 += s2 * (Setting.getDouble("bat/isotope/calc/nominal_ra")/corr.std.std_ra) ;
			s8 += Math.pow(s7*corr.std.std_ra_err/corr.std.std_ra, 2);
//			log.debug("s8: "+s8);
		...
		result[0] = s7 / weight;		// mean value
		result[1] = Math.sqrt((s6+s8)/Math.pow(weight,2)+Math.pow(Setting.getDouble("/bat/isotope/calc/scatter")*result[0],2));  // error
//		log.debug("(s6+s8):"+(s6+s8));
//		log.debug("Math.pow(weight,2):"+Math.pow(weight,2));
//		log.debug("Math.sqrt((s6+s8)/Math.pow(weight,2):"+Math.sqrt((s6+s8)/Math.pow(weight,2)));
		for (int i=0; i<runs.size(); i++) {
			if (runs.get(i).active) {
				dev += runs.get(i).a * runs.get(i).runtime * Math.pow(runs.get(i).ra_fin-result[0],2);
				k++;
		...
		for (int i=0; i<data.size(); i++) {
			try {
				if(data.get(i).active()) {
					regression.addData((Double)data.get(i).get(x), (Double)data.get(i).get(y));}
				}
			catch (NullPointerException e) {;}
		}
	 ...
	 /**
		...
		public static XYDataset getXY(ArrayList<DataSet> list, Integer corrIndex, String x, String y, int x_multi, int y_multi){
		 ...
		 /**
		     ...
		     * @return xy dataset for JFreePlot
		     */
			public static IntervalXYDataset getXY_err(ArrayList<DataSet> list, Integer corrIndex, String x, String y, int x_multi, int y_multi){
				...
				{
					lastLabel = (String) list.get(0).get("label");
					YIntervalSeries series = new YIntervalSeries(lastLabel);
					...
					for (int k=0; k<list.size(); k++)
					{
						if (corrIndex==null||list.get(k).get("corrIndex").equals(corrIndex)) {
							if (list.get(k).get("active").equals(true)) {
								...
								if (!thisLabel.equals(lastLabel)) {
									dataSet.addSeries(series);
									series = new YIntervalSeries(/*((String)list.get(k).get("commentinfo"))+"-"+*/thisLabel);
									lastLabel = (String) list.get(k).get("label");
								}
								try {
									series.add(
										((Double)list.get(k).get(x) * x_multi),
										(Double) list.get(k).get(y) * y_multi,
										((Double) list.get(k).get(y)-(Double) list.get(k).get(y+"_err")) * y_multi,
										((Double) list.get(k).get(y)+(Double) list.get(k).get(y+"_err")) * y_multi
											);
								}
								catch (ClassCastException e)
			 ...
			 * @return xy dataset for JFreePlot
			 **/
				public static IntervalXYDataset getXY_cyc(ArrayList<Cycle> list, String serie1, String serie2){
					...
					if (list!=null&&list.size()>0) 
					{
						...
						for (int i=0; i<list.size(); i++) {
									try {
										series.add(
											(list.get(i).cycle.doubleValue()) ,
											(Double) list.get(i).a );
										series2.add(
											(list.get(i).cycle.doubleValue()) ,
											((Double) list.get(i).r / max2 * max));
									}
									catch (ClassCastException e)
									{
										log.warn("Field must be a number!(CalcData)");
				 ...
				 * @return xy dataset for JFreePlot
				 **/
					public static IntervalXYDataset getXY_act(ArrayList<Cycle> list, String serie1){
						...
						if (list!=null&&list.size()>0) 
						{
							...
							for (int k=0; k<list.size(); k++) {
										try {
											if (list.get(k).get("active").equals(false)) {
											    series.add(
												(list.get(k).cycle.doubleValue()) ,
												((Double) max));
											}
										}
										catch (ClassCastException e)
...
//									} else {;}
				 ...
				 /**
				     ...
				     * @return xy dataset for JFreePlot
				     */
					public static IntervalXYDataset getXY_cyc2(ArrayList<Cycle> list, String serie){
						XYSeriesCollection dataSet = new XYSeriesCollection();
