...
public class DataTable extends JPanel {
	...
	private static final Logger log = LogManager.getLogger("DataTable");
     ...
     * @param mean 
     */
    public DataTable(Bats main, Calc data, String type, final FormatT format, Boolean mean) {
...
//		table.setPreferredScrollableViewportSize(new Dimension(1000, 70));
    ...
    class MyTableModel extends AbstractTableModel { 	
		...
		public boolean isCellEditable(int row, int col) {
			...
			return true;
		    } else if (format.colEdit.get(col)==true) {
				if ((data.get(type).get(row).active()||format.colValue.get(col).equals("active"))&&(data.get(type).get(row).editable())) {
					return true;
				}
				else return false;
        	} 
	...
	public void setValueAt(Object value, int row, int col) {
            ...
            log.debug(data.get(type).get(row).get(format.colValue.get(col)));
                if (data.get(type).get(row).getClass().equals(Sample.class)) {
                	if ((format.colValue.get(col).equals("label") || format.colValue.get(col).equals("comment"))) {
        				...
        				Sample sample = (Sample)data.get(type).get(row);
        				try {
            				CalibGraph graph = new CalibGraph((sample.sample_nr.toString()), sample.age, sample.age_err, main.calib);
            				graph.displayAuto();
            			} catch (NullPointerException e) {
                ...
                if ((format.colValue.get(col).equals("run") && data.get(type).get(row).getClass().equals(Run.class))) {  //  && cycEdit
                    log.debug("Edit run: "+data.get(type).get(row).get("run"));
	       	    main.cycDat.getCycleTable((Run)data.get(type).get(row));
	       	} else if ((Integer)(data.get(type).get(row).get("editable"))<2) {
