...
{ 	
	...
	private static final Logger log = LogManager.getLogger("Calib");	
...
//						log.debug("Skip line "+i+": "+line);
			...
			if (calArr.size()>0) {
				...
				String message = String.format( "No records found in file for calibration!" );
				JOptionPane.showMessageDialog( null, message );
				log.warn("NumberFormatException: "+message);
			}
		}
...
//			log.debug(df);
			if (dt<=0 && !error) {
				log.debug("smaller: p0:"+p0.calAge);
				String message = String.format( "Calibration file not properly formated (Calendar age "+p0.calAge+" is equal to "+p1.calAge+" or larger). Calibration not possible!" );
				JOptionPane.showMessageDialog( null, message );
				error=true;
			}
...
//			log.debug(rSig);
		...
		Collections.sort(probArr, new SortAge()); 		
		return probArr;
	}
...
//			    log.debug(unc);
//			    log.debug(sig);
//			    log.debug(getProbability(i, unc).size());
//			    log.debug(sigmaProba(getProbability(i, unc), sig).size());
...
//				    pw.println();	
	 ...
	 * @param inc
	 */
	public void statistic2(double unc, int inc) {
		Range range;
		
		String file = Setting.homeDir+"/statistics-"+unc+"-dist.txt";
		if (new File(file).renameTo(new File(new File(file)+".old"))) { 
			log.debug("Old file renamed!");
		}
		try {
			...
			for (int i=50; i<=14000; i=i+inc) {
			    log.debug(calArr.get(i).radAge);
				range = getRange(sigmaProba(getProbability(calArr.get(i).radAge, unc),2));
				    pw.print(i+"\t");
				    pw.print(range.ll+"\t");
				    pw.print(range.hl+"\t");
				    pw.println();
	...
	public boolean manInput() {
		...
		namePanel.setLayout(new GridLayout(0, 1));
 		// Create the labels
		JLabel name = new JLabel("Sample name:     ", SwingConstants.RIGHT);
		JLabel  age = new JLabel("Radiocarbon age: ", SwingConstants.RIGHT);
		JLabel  sig = new JLabel("Uncertainty:     ", SwingConstants.RIGHT);
		namePanel.add(name);
		namePanel.add(age);
		namePanel.add(sig);
