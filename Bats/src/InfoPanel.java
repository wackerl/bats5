...
public class InfoPanel extends JPanel {
	 ...
	 * 
	 */
	private static final Logger log = LogManager.getLogger("InfoPanel");
...
//     		if (Setting.getDouble("/bat/isotope/calc/scatter")!=0.0) {
	     		label = new JLabel("Target scatter: "+df.format(Setting.getDouble("/bat/isotope/calc/scatter")*100)+" %");
	    		label.setName("calc");
	     		label.addMouseListener(this);
	    		label.setForeground(cText);
	    		label.setFont(fText);
...
//     		}
    		...
    		if (Setting.isotope.contains("C14")) {
    		    df = new DecimalFormat("0.0");
    	 		label = new JLabel("Nom. F¹⁴C: "+df.format(Setting.getDouble("/bat/isotope/calc/nominal_ra"))+" ("+df.format(Setting.getDouble("/bat/isotope/calc/nominal_ba")*1000-1000)+"‰)");
    	 		label.setToolTipText("Nominal F¹⁴C value of standard. Standards are normalised to this value.");
