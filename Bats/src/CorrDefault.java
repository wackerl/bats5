 ...
 *
 */
public class CorrDefault extends Corr {
	private static final Logger log = LogManager.getLogger("CorrDefault");
	
	CorrDefault(StdNom stdNom) {
		super(stdNom);
		blank.ra_bg = -1.0;		
		std.std_ra = -1.0;
		std.std_ra_err = -1.0;
	...
	@Override
	public Corr copy() {
		Corr corr = new CorrDefault(this.stdNom);
...
//cv
		...
		for (int i=0; i<sampleList.size(); i++) {
			...
			try {	
//cv orig				sample.std_ba = sample.ba_cur * Setting.getDouble("bat/isotope/calc/nominal_ba") / (1+stdData.delta/1000);
//				sample.std_ba_err = sample.ba_cur_err * Setting.getDouble("bat/isotope/calc/nominal_ba") / (1+stdData.delta/1000);
//cv orig				sample.std_ba_sig = sample.ba_cur_sig * Setting.getDouble("bat/isotope/calc/nominal_ba") / (1+stdData.delta/1000); 
//				log.debug("nominal_ba: "+Setting.getDouble("bat/isotope/calc/nominal_ba"));
//				log.debug("sample.ba_cur: "+sample.ba_cur);
//				log.debug("sample.ba_cur_err: "+sample.ba_cur_err);
//				log.debug("sample.ba_cur_sig: "+sample.ba_cur_sig);
				sample.std_ba = sample.ba_cur; // Setting.getDouble("bat/isotope/calc/nominal_ba");
				sample.std_ba_err = sample.ba_cur_err; // Setting.getDouble("bat/isotope/calc/nominal_ba");
				sample.std_ba_sig = sample.ba_cur_sig; // Setting.getDouble("bat/isotope/calc/nominal_ba"); 
...
//cv orig				run.std_ba = run.ba_cur * Setting.getDouble("bat/isotope/calc/nominal_ba") / (1+stdData.delta/1000);
//				run.std_ba_err = run.ba_cur_err * Setting.getDouble("bat/isotope/calc/nominal_ba") / (1+stdData.delta/1000);
//				try {
//cv orig					run.std_ba_sig = run.ba_cur_sig * Setting.getDouble("bat/isotope/calc/nominal_ba") / (1+stdData.delta/1000);
				run.std_ba = run.ba_cur; // Setting.getDouble("bat/isotope/calc/nominal_ba");
				run.std_ba_err = run.ba_cur_err; // Setting.getDouble("bat/isotope/calc/nominal_ba");
				try {
					run.std_ba_sig = run.ba_cur_sig; // Setting.getDouble("bat/isotope/calc/nominal_ba");
				} catch (NullPointerException e){
					run.std_ba_sig = 0.0;
				}	
			}
...
//cv orig				log.debug("std 13C set: "+std.std_ba+"±"+std.std_ba_err);
				log.debug("std.ba set: "+std.std_ba+"±"+std.std_ba_err);
...
//		double s_r  = 0.0;
		...
		for (int i=0; i<runList.size(); i++) {
			...
			if (run.active == true && run.std_ra!=null){
				p = run.a_cur * run.runtime*stdData.ra;			
				s1 += p;
				s2 += p * run.std_ra;
		...
		for (int i=0; i<sampleList.size(); i++) {
			...
			try {
				...
				sample.std_ra_err = Math.sqrt(Math.pow(sample.ra_bg_err*on_corr,2)
...
//							log.debug(runList.get(j).active+"--"+runList.get(j).std_ra);
						}
					}
				}
				if (runLS.size()>1) {
...
//				sample.std_ra_sig = Math.sqrt(Math.pow(sample.ra_bg_sig,2)+Math.pow(blank.ra_bg_sig,2)) *on_corr;
			...
			if (sample.active==true && sample.std_ra!=null){
				p = sample.a_cur * sample.runtime*stdData.ra;	// weight
		...
		if(k>1) {
			...
			stdSc.std_ra_sig = Math.sqrt( (s3 - Math.pow(s2,2)/s1) / (s1 *(k-1)) );
		} else if (k==1) {
