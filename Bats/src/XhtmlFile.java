...
public class XhtmlFile {
	private static final Logger log = LogManager.getLogger("XhtmlFile");
	/**
	 * 
	 */
...
//				log.debug("Sample size"+dat.size());
			    for (int i=0; i<dat.size(); i++){
			    	if (dat.get(i).get("active").equals(true)) {
				    	String cl="", cl2="";
				    	if (data.stdNom.isBl(dat.get(i).get("sampletype").toString().toLowerCase()) ) {
							cl="bl";
				    	...
				    	for (int k=0; k<format.colValue.size(); k++) {
							...
							if (format.colFormat.get(k)!="" && (value instanceof Number)) {
								...
								align="left";
//								try {log.debug(value.getClass());} catch (Exception e) {;}
//								log.debug(value);
							}
				...
				ArrayList<DataSet> dat = data.runSampleList;
			    ...
			    for (int i=0; i<dat.size(); i++){
			    	if (dat.get(i).get("active").equals(true)) {
				    	...
				    	for (int k=0; k<format.colValue.size(); k++) {
							...
							try {
				    		pw.print("<td align=\""+align+"\">"+((String)value).replaceAll(">","&#62").replaceAll("<","&#60").replaceAll("&","&amp;>")+"</td>");
							}
	    ...
	    // corrections
		    ...
		    if (Setting.getBoolean("/bat/general/file/output/corr")== true) {
				...
				if (Setting.isotope.contains("C14")) {
					pw.print("    <tr><td>Nominal value: </td><td>"+Setting.getDouble("bat/isotope/calc/nominal_ra")+"</td><td align=\"left\">F<sup>14</sup>C</td></tr>");
