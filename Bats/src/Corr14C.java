...
public class Corr14C extends Corr {
	private static final Logger log = LogManager.getLogger("Corr14C");
...
//				p = sample.a_cur * sample.runtime;	// weight
		...
		for (int i=0; i<runList.size(); i++) {
			...
			if (run.sample.active == true && run.active == true && run.std_ba!=null){
...
//				Double b = Math.max(stdS.std_ba_sig, stdS.std_ba_err);
		...
		for (int i=0; i<runList.size(); i++) {
			...
			if (run.ra_bg!=null){				
				...
				on_corr = Setting.getDouble("bat/isotope/calc/nominal_ra")/(stdData.F14C);
			...
			if (run.sample.active == true && run.active == true && run.std_ra!=null){
				p = run.a_cur * run.runtime * stdData.F14C;			
				s1 += p;
				s2 += p * run.std_ra;
...
//				sample.std_ra_sig = Math.sqrt(Math.pow(sample.ra_bg_sig,2)+Math.pow(blank.ra_bg_sig,2)) *fract_corr*on_corr;
			...
			if (sample.active==true && sample.std_ra!=null){
				p = sample.a_cur * sample.runtime*stdData.F14C;	// weight
...
//			p = 1.0;
	...
	blankS.number=k;
	if(k>1) {
		blankS.ra_bg_sig = Math.sqrt( (s3 - Math.pow(s2,2)/s1) / (s1*(k-1)/k) );
	...
	for (int i=0; i<runList.size(); i++) {
		run = runList.get(i);
		if (runList.get(i).sample.active==true && runList.get(i).active==true) {
