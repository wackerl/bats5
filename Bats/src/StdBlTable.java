...
{
	private static final Logger log = LogManager.getLogger("StdBlTable");
	StdBlGraph plot;
	Calc data;
	Bats main;
     ...
     * @param format 
     */
	public StdBlTable(Bats main, Calc data, String type_o, final FormatT format) {
...
//	        header.setReorderingAllowed(false);
		header.addMouseListener(new MouseAdapter() {  
	        	...
	        	if (e.getButton()==3) {
	        	log.debug("Change page setup of "+format.file.getPath());
	        	format.showSettings();
	        	}
	            }  
	        });
		...
		{	
			...
			try
			{
				if (cell instanceof Double) {
					...
					cell = new JButton((String)data.get(type).get(row).get(format.colValue.get(col)));
				}
...
//		log.debug(data.get(type).get(row).editable()+"  "+data.get(type).get(row).get("label")+"  "+data.get(type).get(row).getClass());
		    if (data.get(type).get(row) instanceof Run  &&  (format.colValue.get(col).equals("run"))) {  //  && cycEdit
			return true;
		    } else if (data.get(type).get(row) instanceof StdBl  &&  (format.colValue.get(col).equals("run"))) {  //  && cycEdit
			return true;
		    } else if ( data.get(type).get(row).editable() )
			{
				if (data.get(type).get(row) instanceof Run || data.get(type).get(row) instanceof Sample)
				{
					if (format.colValue.get(col).equals("active")&&(data.get(type).get(row).editable())) {
...
//				}
//			}
	       	if ((format.colValue.get(col).equals("run") && data.get(type).get(row).getClass().equals(Run.class))) {  //  && cycEdit
	       	    main.cycDat.getCycleTable((Run)data.get(type).get(row));
	       	} else if (data.get(type).get(row).getClass().equals(StdBl.class) ) { 
	       	    log.debug("Edit of StdBl class");
	       	    if (((DataSet)data.get(type).get(row)).get("run").equals("Mean")) {
	       		log.debug("Mean");
			  if (((DataSet)data.get(type).get(row)).get("label").equals("of runs")) {
//			      log.debug(((DataSet)data.get(type).get(row)).get("label"));
			      JOptionPane.showMessageDialog( null, "Mean based on runs, weighted with the errors of the individual runs (excl. sample scatter!).");
			  } else  if (((DataSet)data.get(type).get(row)).get("label").equals("of samples")) {
			      JOptionPane.showMessageDialog( null, "Mean based on samples, weighted with the errors of the individual samples (incl. sample scatter!).");
			  } else if (((DataSet)data.get(type).get(row)).get("label").equals("sample scatter")) {
			      JOptionPane.showMessageDialog( null, "Mean based on samples, sigma is the scatter.");
			  }
	       	    } else {
			log.debug(format.colValue.get(col));
			log.debug(col);
			...
			if (Setting.getBoolean("/bat/general/calc/autocalc")) {
	       	...
	       	} else if (format.colValue.get(col).equals("a") || format.colValue.get(col).equals("b") || format.colValue.get(col).equals("label")
...
//		        				}
						...
						((Sample)data.get(type).get(row)).set(value, format.colValue.get(col));
	    				log.debug("Set "+data.get(type).get(row));
					}
					...
					if (Setting.getActive("/bat/isotope/db")) {
        		        		if (main.db.runTrue((Run)data.get(type).get(row), (Boolean)value)) {
        		        			...
        		        			if ((Boolean)value) {
        		        			    if (((Run)data.get(type).get(row)).sample.runs==0) {
        			        			    ((Run)data.get(type).get(row)).sample.set(value, "active");	
        		        			    }
        		        			} else {
        		        			    if (((Run)data.get(type).get(row)).sample.runs==1) {
        			        			    ((Run)data.get(type).get(row)).sample.set(value, "active");	
        		        			    }		        			    
        		        			}		        			   	
        		        		} else {
...
//					  isSelected, hasFocus, row, column);
			  ...
			  if (isSelected) {
				  ...
				  if (data.get(type).get(row).get("label").equals("used")) {
			  ...
			  if (!data.get(type).get(row).editable()) {
				  if (isSelected) {
					  foreground = Setting.getColor("/bat/general/table/color/fg_select");
					  background = Setting.getColor("/bat/general/table/color/bg_select");
			          }else {
...
//					  if (((DataSet)data.get(type).get(row)).get("label").equals("of runs")) {
//					      log.debug(((DataSet)data.get(type).get(row)).get("label"));
//					      this.setToolTipText("Mean based on runs, weighted with the errors of the individual runs (excl. sample scatter!).");
//					  } else  if (((DataSet)data.get(type).get(row)).get("label").equals("of samples")) {
//					      this.setToolTipText("Mean based on samples, weighted with the errors of the individual samples (incl. sample scatter!).");
//					  } else if (((DataSet)data.get(type).get(row)).get("label").equals("sample scatter")) {
//					      this.setToolTipText("Mean based on samples, error is sample scatter.");
//					  }
...
//				  table.getColumnModel().getColumn(column).setCellEditor(new DefaultCellEditor(format(numberValue.doubleValue()));
				  ...
				  if (format.colChi.get(column)==true) {
					  int a = data.get(type).get(row).number()-1;
			  ...
			  } else if (data.get(type).get(row).get("label").equals("used")) {
		    	  	((JComponent) renderer).setBorder(BorderFactory.createMatteBorder(1,0,2,0,Setting.getColor("/bat/general/table/color/line_sample")));
			  }
			  return renderer;
		  }
