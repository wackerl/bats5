...
public class StdNomTable extends JPanel {
	private static final Logger log = LogManager.getLogger("StdNomTable");
	...
	public StdNomTable(StdNom stdNom) {
		this.stdNom = stdNom;
		setIsotope(Setting.isotope);
		myTableModel = new MyTableModel();
	    JTable table = new JTable(myTableModel);
	    log.debug("Crate table for std/bl");
	    // Table settings
    ...
    class MyTableModel extends AbstractTableModel { 	
		...
		public Object getValueAt(int row, int col) {
			try {return stdNom.stdList.get(row).get(columnField.get(col));
			} catch (NullPointerException e) {
			    log.warn("standard file not properly formated");
			    return "";
			}
		}
