...
class DbMySqlConnect implements DbConnect {
    private static final Logger log = LogManager.getLogger("DbMySqlConnect");
    Integer year;
    private Bats main;
...
//				log.debug("sql querry to get run was successful: "+query);
			...
			if (login()) {
				...
				for ( int i=0; i<magList.size(); i++) {
					if (magList.get(i).magazine!=null) {
						...
						magInfo[k++] = (magList.get(i).magazine+"            ").substring(0,12)
									+ " | "
	...
	private Boolean login() throws HeadlessException, SQLException {
    	if (conn==null || !conn.isValid(5)) {
			...
			pw = new String(Base64.decodeBase64(Setting.getString("/bat/isotope/db/sql-import/pw").getBytes()));
			...
			if(JOptionPane.showOptionDialog(
				...
				try {
				    ...
				    conn = DriverManager.getConnection("jdbc:"+url+"?user="+id+"&password="+pw/* + "?autoReconnect=true"*/);
				    log.debug(conn);
//					conn..sets.query("SET GLOBAL connect_timeout=2000");
				} catch (SQLException e) {
